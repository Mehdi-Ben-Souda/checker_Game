 ...
 * sorties : void
 */
void creer_fils(xmlDocPtr doc, xmlNodePtr cur, GtkWidget* Gtk_parent)
{
    int nat,n=0;
    ...
    while (cur != NULL) {
        ...
        switch (nat)
        {
            ...
            case 7:
                ...
                gtk_fixed_put(GTK_FIXED(Gtk_parent), GTK_WIDGET(toolbar->toolbar),
            ...
            case 8 :
                do
                {
                    ...
                    ToolItem = Init_CelluleTooolItem((char*)xmlGetProp(cur, "label"),
                                                            (char*)xmlGetProp(cur, "icon"));
                    ToolItem = Creer_CelluleToolItem(ToolItem);//creer le nouvel élément
                    Inserer_Tool_Item(Gtk_parent,ToolItem,&n);
...
void Lire_doc(char* docname) {
    ...
    //pointer sur le fils de la balise courante
    cur = cur->xmlChildrenNode;
    //boucler jusqu'il n'y a aucune balise à traiter (frère)
    while (cur != NULL) {
