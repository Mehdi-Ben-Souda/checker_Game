 ...
 */
ToolItem *Init_CelluleTooolItem(char label[NB_Cara_titre],
                                char icon[NB_Cara_chemin])
{
    ToolItem *NE; // déclaration d'un nouvel élément
    // l'allocation de la mémoire
    NE = (ToolItem *)malloc(sizeof(ToolItem));
 ...
 * entrées : -> un pointeur sur la liste des éléments
                construisant le toolbar
...
ToolBar *Creer_toolbar(ToolBar *toolbar)
{
    ...
    gtk_toolbar_set_icon_size(GTK_TOOLBAR(toolbar->toolbar),
                              toolbar->icon_size);
    // mettre le style des éléments
    gtk_toolbar_set_style(GTK_TOOLBAR(toolbar->toolbar),
                          toolbar->style);
